...
class TestDatabaseFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from src.bot.discord_aiavatar_complete import init_db, save_message, get_user_history, \
                                                       set_user_prompt, get_user_prompt, \
                                                       set_recording_enabled, get_recording_settings, \
...
class TestOnMessageFunctionality(unittest.TestCase):
    ...
    @patch('src.bot.discord_aiavatar_complete.logger') # Mock the logger instance used in the module
    @patch('src.bot.discord_aiavatar_complete.aiavatar') # Mock the global aiavatar instance
    @patch('src.bot.discord_aiavatar_complete.DB_NAME', ':memory:')
    def setUp(self, mock_db_name, mock_global_aiavatar, mock_logger, mock_ffmpeg_audio, mock_temp_file, mock_os_remove, mock_asyncio_sleep):
        # Store mocks from arguments
        self.mock_db_name_val = mock_db_name # Though :memory: is directly used by the module
        ...
        from src.bot.discord_aiavatar_complete import (
            AIAvatarBot, init_db, save_message, get_user_history,
            get_user_prompt, DEFAULT_SYSTEM_PROMPT
        )
        ...
        self.mock_global_aiavatar_instance.tts.speak = AsyncMock(return_value=b"test_tts_audio_data")
        ...
        self.mock_temp_file_object.__enter__.return_value = self.mock_temp_file_object # For context manager
    ...
    async def create_mock_message(self, content, author_is_bot=False, guild_name="TestGuild", channel_send_mock=None, author_id=123, author_name="TestUser"):
    ...
    # 1. Message from Bot
    @patch('src.bot.discord_aiavatar_complete.DB_NAME', ':memory:') # ensure DB is patched for module level if test runs standalone
    async def test_on_message_ignores_bot_author(self, mock_db_name_override=None): # mock_db_name_override to consume the patch if method is run standalone
    ...
    @patch('src.bot.discord_aiavatar_complete.DB_NAME', ':memory:')
    async def test_on_message_successful_text_to_voice(self, mock_db_name_override, mock_save_message):
        ...
        mock_message = await self.create_mock_message("Test query", channel_send_mock=mock_channel_send, author_id=999)
    ...
    async def test_on_message_llm_error(self, mock_db_name_override, mock_save_message):
        ...
        mock_channel_send = AsyncMock()
        mock_message = await self.create_mock_message("Query causing LLM error", channel_send_mock=mock_channel_send, author_id=998)
        ...
        self.mock_logger_instance.error.assert_any_call(f"Error during AI processing for {mock_message.author.name} (ID: {mock_message.author.id}): LLM processing failed")
    ...
    async def test_on_message_tts_error(self, mock_db_name_override, mock_save_message):
        ...
        self.bot.voice_clients = [mock_vc]
        mock_channel_send = AsyncMock()
        mock_message = await self.create_mock_message("Query causing TTS error", channel_send_mock=mock_channel_send, author_id=997)

        self.mock_global_aiavatar_instance.llm.chat.return_value = "Successful LLM response for TTS test"
        ...
        self.assertEqual(mock_save_message.call_count, 2) 
        mock_channel_send.assert_called_once_with("Successful LLM response for TTS test") # Text response still sent
        self.mock_global_aiavatar_instance.tts.speak.assert_called_once()
        mock_vc.play.assert_not_called()
        self.mock_logger_instance.error.assert_any_call(f"Error during TTS or audio playback for {mock_message.author.name}: TTS failed")
    ...
    async def test_on_message_channel_send_error(self, mock_db_name_override, mock_save_message):
        ...
        mock_channel_send_that_fails = AsyncMock(side_effect=discord.DiscordException("Failed to send message"))
        mock_message = await self.create_mock_message("Query leading to send error", channel_send_mock=mock_channel_send_that_fails, author_id=996)

        self.mock_global_aiavatar_instance.llm.chat.return_value = "LLM response for send error test"
        ...
        self.mock_logger_instance.error.assert_any_call(f"Failed to send AI response to channel {mock_message.channel.name} for {mock_message.author.name}: Failed to send message")
        
        # TTS should still be attempted as per current logic
        self.mock_global_aiavatar_instance.tts.speak.assert_called_once_with("LLM response for send error test")
        ...
        with patch('dotenv.load_dotenv', return_value=True):
            ...
            with patch('logging.getLogger', return_value=MagicMock()): # General logging
                 with patch('discord.Client', MagicMock()): # If AIAvatarBot's super uses it
                    with patch('discord.app_commands.CommandTree', MagicMock()):
                        with patch('src.bot.services.ai_service.AIAvatar', MagicMock()): # If this is a different AIAvatar
                             unittest.main()
