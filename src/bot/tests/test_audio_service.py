...
class TestAudioService(unittest.TestCase):
    ...
    def test_record_stops_after_max_duration(self, mock_input_stream_class, mock_sf_write, mock_temp_file):
        # Configure mocks
        ...
        samplerate = 16000
        channels = 1
        audio_chunk = np.array([[0.5]] * samplerate, dtype=np.float32) # 1 second of non-silent audio
        ...
        filename = record_with_silence_detection(samplerate=samplerate, channels=channels, silence_threshold=0.01, silence_duration=2, max_duration=5)

        self.assertEqual(filename, "test_max_duration.wav")
        mock_input_stream_class.assert_called_once_with(samplerate=samplerate, channels=channels, dtype='float32', blocksize=samplerate // 10) # blocksize is samplerate // 10
    ...
    def test_record_stops_due_to_silence(self, mock_time, mock_input_stream_class, mock_sf_write, mock_temp_file):
        # Configure mocks
        mock_stream_instance = MagicMock()
        mock_input_stream_class.return_value = mock_stream_instance
        ...
        num_silence_blocks_needed = (silence_duration_config * samplerate) // blocksize + 1 # +1 to ensure duration is met
        ...
        for i in range(num_initial_sound_blocks + num_silence_blocks_needed + 5): # Add a few extra ticks
        ...
        mock_input_stream_class.assert_called_once_with(samplerate=samplerate, channels=channels, dtype='float32', blocksize=blocksize)
        ...
        expected_frames = (num_initial_sound_blocks + num_silence_blocks_needed -1) * blocksize # -1 because the loop breaks after condition is met
        ...
        self.assertGreaterEqual(len(args[1]), (num_initial_sound_blocks + num_silence_blocks_needed - 2) * blocksize ) # Allow some leeway
        self.assertLessEqual(len(args[1]), (num_initial_sound_blocks + num_silence_blocks_needed + 2) * blocksize )
