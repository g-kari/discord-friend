...
try:
    ...
    if not TOKEN:
        ...
        if "pytest" not in sys.modules:
            raise ValueError("Discord Botトークンが設定されていません。.envファイルを確認してください。")
    ...
    def get_user_prompt(user_id):
        try:
            conn = get_db_connection()
            c = conn.cursor()
            c.execute("SELECT prompt FROM system_prompts WHERE user_id = ?",
                      (user_id, ))
            row = c.fetchone()
            if row:
    ...
    def get_recording_settings(user_id):
        try:
            ...
            if not row:
                ...
                logger.debug(f"ユーザーID {user_id} の録音設定が見つからないため、デフォルト設定（有効）を適用しました")
                result = True, None
            else:
                result = row[0], row[1]
        ...
        async def on_message(self, message: discord.Message):
            ...
            if (isinstance(message.content, str)
                ...
                if message.content.startswith(
                    ...
                    if (isinstance(message.author, discord.Member)
                        ...
                        if set_default_system_prompt(prompt):
                            ...
                            await message.channel.send("デフォルトのシステムプロンプト設定中にエラーが発生しました。")
                    else:
                ...
                # Check for get_default_prompt command
                    ...
                    await message.channel.send(f"現在のデフォルトシステムプロンプト：\n```{prompt}```")
            ...
            if (voice_client_in_guild and isinstance(voice_client_in_guild, VoiceClient)
                    ...
                    if ai_response:  # Only save if a response was generated
                        ...
                        if (voice_client_in_guild
                            ...
                            try:
                                ...
                                if aiavatar is None:
                                    logger.error("AIAvatarが初期化されていないため、TTS処理をスキップします")
                                    tts_audio_data = b""
                                else:
                                    # 会話クッションを送信しながらTTS実行
                                    from services.ai_service import create_cushion_task

                                    cushion_task, cancel_event = (await
                                                                  create_cushion_task(
                                                                      message.channel))
                                    try:
                                ...
                                with tempfile.NamedTemporaryFile(
                                        suffix=".wav", delete=False) as tmp_tts_file:
                                ...
                                # 3. Play Audio
                                if (isinstance(voice_client_in_guild, VoiceClient)
                                        and voice_client_in_guild.is_playing()):
                                    logger.warning(
                                        f"Voice client is currently playing audio for {username}. "
                                        "Stopping previous audio.")
                                    voice_client_in_guild.stop(
                                    )  # Stop current playback
                                    await asyncio.sleep(
                                        0.1)  # Short pause to allow stop to take effect

                                channel_name = (voice_client_in_guild.channel.name if
                                                hasattr(voice_client_in_guild.channel,
                                                        "name") else "unknown")
                                logger.info(
                                ...
                                while (isinstance(voice_client_in_guild, VoiceClient)
                                       and voice_client_in_guild.is_playing()):
    ...
    try:
        if AIAvatar is not None:
            aiavatar = AIAvatar(openai_api_key=OPENAI_API_KEY, )
        else:
    ...
    async def on_voice_state_update(member, before, after):
        ...
        if after.channel and (not before.channel  # チャンネル参加
        ...
        if isinstance(interaction.user, discord.Member) and interaction.user.voice:
            ...
            if channel is not None:  # Add null check for channel
                ...
                if hasattr(voice_client, "listen"):  # Type check
                    ...
                    setattr(voice_client, "sink",
                            audio_sink)  # Use setattr instead of direct assignment
            ...
            logger.info(f"ユーザー「{interaction.user.display_name}」の録音設定を自動的にオンにしました")
            ...
            if channel is not None and hasattr(channel, "members"):
                for member in channel.members:
                    if not member.bot:  # ボットは除外
                        ...
                        logger.info(f"チャンネル内のユーザー「{member.display_name}」の録音設定をオンにしました")
    ...
    async def set_prompt(interaction: discord.Interaction, prompt: str):
        ...
        await interaction.response.send_message(f"システムプロンプトを設定しました。\n```{prompt}```")
    ...
    async def set_default_prompt(interaction: discord.Interaction,
                                 prompt: str,
        ...
        try:
            ...
            if add_to_config:
                ...
                # 環境変数ファイルを更新
                result = env_manager.update_env_variable(key="DEFAULT_SYSTEM_PROMPT",
                                                         value=prompt)
                ...
                if result:
                    ...
                    logger.warning("デフォルトシステムプロンプトを保存する.envファイルが見つからないか、更新できませんでした")
                    await interaction.response.send_message(
    ...
    async def get_default_prompt(interaction: discord.Interaction):
        prompt = get_default_system_prompt()
        await interaction.response.send_message(f"現在のデフォルトシステムプロンプト：\n```{prompt}```")
    ...
    async def recording_on(interaction: discord.Interaction,
        ...
        if keyword:
            ...
            await interaction.response.send_message("録音をオンにしました。あなたが話し始めたら自動で応答します。")
    ...
    async def recording_off(interaction: discord.Interaction):
        ...
        await interaction.response.send_message("録音をオフにしました。あなたの声には反応しなくなります。")
    ...
    async def add_mcp_server(interaction: discord.Interaction,
                             add_to_config: bool = False):
        ...
        if (not isinstance(interaction.user, discord.Member)
                or not interaction.user.voice or not interaction.user.voice.channel):
            await interaction.response.send_message("先にボイスチャンネルに参加してください。")
            return
        ...
        try:
            ...
            if channel_name not in MCP_SERVERS[guild_name]:
                ...
                if add_to_config:
                    # 環境変数ファイルを更新
                    result = env_manager.update_env_variable(key="MCP_SERVERS",
                                                             value=MCP_SERVERS,
                                                             json_encode=True)
            ...
            await interaction.response.send_message(f"MCPサーバーの追加中にエラーが発生しました: {str(e)}")
    ...
    async def list_mcp_servers(interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("**MCPサーバーリスト**\n" +
                                                    "\n".join(server_list))
        except Exception as e:
    ...
    async def remove_mcp_server(
        ...
        try:
            ...
            await interaction.response.send_message(f"MCPサーバーの削除中にエラーが発生しました: {str(e)}")
    ...
    async def talk(interaction: discord.Interaction):
        ...
        if (not isinstance(interaction.user, discord.Member)
                or not interaction.user.voice or not interaction.user.voice.channel):
            await interaction.response.send_message("ボイスチャンネルに接続してから使用してください。")
            return
        ...
        if (not isinstance(interaction.user, discord.Member)
            ...
            await interaction.response.send_message("ボットと同じボイスチャンネルに接続してください。")
            return
        ...
        await interaction.response.send_message("会話を開始します。話してみてください。画面共有にも対応しています。")
            ...
            for user_id in users:
                try:
                    ...
                    user_name = (f"{discord_user.name}#{discord_user.discriminator}"
                                 if discord_user else f"Unknown ({user_id})")
    ...
    async def admin_prune_history(interaction: discord.Interaction, days: int):
        ...
        try:
            ...
            if deleted_count >= 0:
                ...
                await interaction.followup.send("会話履歴の削除中にエラーが発生しました。", ephemeral=True)
    ...
    async def trigger_ai_conversation(vc, member):
        ...
        try:
            ...
            for channel in vc.guild.text_channels:
                if channel.permissions_for(vc.guild.me).send_messages:
                    ...
                    logger.debug(f"録音開始通知をテキストチャンネル「{text_channel.name}」に送信しました")
        ...
        try:
            ...
            logger.info(f"音声合成完了: サイズ {file_size / 1024:.1f} KB (処理時間: {elapsed:.2f}秒)")
            ...
            try:
                if text_channel:  # Use the previously validated text_channel
                    ...
                    logger.info(f"テキストチャンネル「{text_channel.name}」に会話内容を送信しました")
            except Exception as e:
